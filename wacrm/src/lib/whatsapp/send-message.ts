...
import { checkContactRateLimit } from '@/lib/contacts/rate-limit';
...
export async function sendMessageToConversation(
  ...
  // Per-contact rate limit (prevents spam loops from automations/auto-reply)
  const rateLimit = await checkContactRateLimit(contact.id, accountId);
  if (!rateLimit.allowed) {
    throw new SendMessageError(
      'rate_limited',
      `Contact rate limit exceeded. Retry after ${Math.ceil((rateLimit.retryAfterMs ?? 0) / 1000)} seconds.`,
      429
    );
  }
